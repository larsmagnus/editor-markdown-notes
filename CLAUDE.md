...
- `content.tsx` - Layout: renders the toolbar and either the raw `<pre>` or the TipTap editor. `src/hooks/use-note-source.ts` picks the content source (VSCode vs. local)
