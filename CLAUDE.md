...
- `heading/`'s `#`x`level` marker is real leading text, not an attribute (same "syntax becomes content" shape as bold/strike/italic/code). Two consequences unique to headings: ProseMirror's default node view only rebuilds a node's DOM on a type/attrs change, never a text change, so retyping `#` into `##` needs `heading-node-view.ts`'s hand-rolled `NodeView` — its `update()` returns `false` on a level change to force ProseMirror to discard and rebuild with the right tag, since nothing else makes `<h1>` become `<h2>` live. And an `InputRule`'s `range` never includes the trigger character that fired it — the framework intercepts it before it reaches the document — so `heading-input-rule.ts` must insert the trailing space itself; skipping that leaves an incomplete one-character marker that `ensure-heading-marker-plugin.ts`'s safety net then "fixes" by prepending a second one.
- `link/`'s `[text](href "title")` keeps `href`/`title` as real mark attributes (unlike heading's dropped `level`) — an image wrapped in a link has no text content at all to hold literal `[`/`](url)` in, so attrs stay the only source of truth for that case, and double as the seed `link-delimiter-spec.ts` synthesizes fresh delimiter text from for the text case (same role italic's `markup` attribute plays). `createSyncLinkAttrsPlugin` runs the relationship in reverse too, re-reading `href`/`title` back off the literal text whenever it changes — typing directly into a revealed `(url)` is the primary way of editing an existing link, so without it the attrs (`renderHTML`'s `<a href>`, the click handler, the popover's seeded field) would go stale the moment someone edited the URL by hand. A link can't use `escape: false` the way every other delimited mark does to protect its own literal `[`/`]`/`(`/`)` from `markdown-escaping.ts`'s generic rules: `prosemirror-markdown` special-cases an `escape: false` mark by excluding it from a node's open/close count entirely, relying on a fallback that only fires for text nodes — for a link wrapping only an image, `open`/`close` would then silently never be called at all, dropping the link on save. `link-markdown-spec.ts`'s `inLink` flag reproduces the same protection one layer up instead, in `esc()` itself.
