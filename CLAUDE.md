...
**A keystroke behaviour asserted through `editor.commands.keyboardShortcut` is not a test of that keystroke.** It runs the handler inside a command chain whose own transaction is built from the pre-command state, so a handler that dispatches separately has its work silently reverted - passing or failing for reasons the browser never sees. Single-character deletion is worse: no keymap handles it at all, it is the browser's own contenteditable behaviour, so backspacing into a delimiter cannot be observed from vitest. Keys, caret placement and decorations belong in `e2e/`; parsers and `appendTransaction` plugins are what unit tests are good for.
...
- A table whose header row is not row 0 has no GFM syntax, so the serializer writes the whole table out as HTML and the sync replaces the user's markdown with it. `prosemirror-tables` reaches that shape on its own — "add row above" from the header row builds the new row from plain cells — so `table/header.ts` restores the invariant in an `appendTransaction` rather than at any one call site. `table/shape.ts` decides what GFM can express; it is the thing that rejects the shape.
- Only `|` is escaped per-cell rather than globally (`markdown-escaping.ts` reads `inTable`), and cells serialize on `content.size`, not on their text — an image-only cell has no text and used to sync as empty.
...
- `transformPastedHTML` must leave the editor's own clipboard slices alone (`data-pm-slice`). Reinstating a block marker is right for HTML from a browser and wrong for a slice that already carries one as real text - a copied `- item` comes back `- \- item`, a copied `> quote` comes back `> > &gt; Quoted`, and both reach the file.
...
- Mermaid is rendering-only: `codeBlock` keeps its name (so the fenced-block serializer stays attached) while swapping in the `code-block-view.tsx` node view. `render-mermaid.ts` is lazy-imported so a note without diagrams never loads it. The diagram is a `PanZoom` viewport, so a click on it starts a pan and cannot also open the source — `mermaid/toolbar.tsx` owns that, and is the only way in. Mermaid also caps its own SVG at the width its layout pass produced, leaving a diagram unable to grow into the room it has; `globals.css` overrides that cap against the width `diagram-width.ts` reads back out of the markup, rather than rewriting the SVG the copy and export actions hand out verbatim.
...
- YAML frontmatter is stripped before `setContent` and re-attached on sync (`frontmatter.ts`), since markdown-it has no concept of it and would parse `---` as an `<hr>`.
...
- Mount-time decorations must map through transactions, not clear on `docChanged` — startup transactions wipe them. Run-once state belongs on the editor instance and in `panel-state.ts`: TipTap rebuilds the editor, and a window reload or reopening a closed tab rebuilds the webview from frozen HTML. `retainContextWhenHidden` (`markdown-editor-provider.ts`) keeps a backgrounded tab's webview alive instead, at the cost of every open note's TipTap instance, Shiki highlighter and (with the writing checks on) retext worker staying resident for the life of the window.
- Catching a rebuilt page up on the document is the webview's job (`getContent` on boot), never a host-side push on `onDidChangeViewState` — that also hits pages whose context survived, discarding a sync debounce of unsynced keystrokes. Tests pass either way; only the handshake is safe.
- Raw and live editing modes both stay mounted, one hidden behind the other (`editor-body.tsx`), rather than unmounting whichever is off screen — ProseMirror's undo history lives entirely in the page, so unmounting either mode used to discard its history on every toggle.
- `link/` extends the same "syntax becomes real, caret-revealed text" pattern as bold/strike/italic/code (see `formatting/`) to `[text](href "title")` — read that directory's own doc comments for the pattern-specific gotchas.
- Every **block** construct's syntax — a heading's `#`s, a blockquote's `> `, a list item's bullet/number/checkbox, a code block's or frontmatter block's fences, a horizontal rule's `---` — goes through `block-marker/`: one `BlockMarkerSpec` per construct in `specs.ts`/`fenced-specs.ts`, and one shared implementation each of repair, reveal, caret handling, Backspace, unwrap, toggle-off strip and paste. Adding a construct means adding a spec, not six files; a defect fixed in the shared pass is fixed for all of them, which is the whole reason the directory exists.
- **Absent syntax is not the same as deleted syntax**, and every repair pass has to ask which it is (`syntax-repair/authored-deletion.ts`, over the batch's own mapping). Absent means a construct arrived without its syntax — an input rule, `splitListItem`, a paste, a toolbar toggle — and the syntax gets written. Deleted means the author removed it, and the construct comes apart instead (`unwrap`, per spec on the block side; `formatting/unwrap-run.ts` on the mark side). Getting this wrong is not a small bug: it makes markdown syntax undeletable, since every backspace against it is answered by a fresh copy. A marker that still _parses_ is always the author's — `###` → `##` is editing a level, not removing a heading — so only syntax that has gone entirely counts as removed.
- A command that deliberately replaces syntax must write the new text in the _same_ transaction (`apply-link-command.ts`). Leaving a bare mark for the repair pass to re-delimit is indistinguishable from an author deleting a delimiter, and gets answered by removing the construct.
- Deleting any character of a mark's delimiter unwraps the whole run — both delimiters and the mark — and both edges answer to both Backspace and Delete (`remove-delimiter-at-boundary.ts`). Bound one edge per direction, the two gestures that actually mean "unformat this" are the two that fall through: backspacing at the end of `_italic_`, and backspacing into a link's `](url)`. A run left carrying one delimiter also serializes as prose reading as half a construct, which markdown-it pairs with whatever follows on the next load.
- A construct that draws something in place of its own marker has to drop it while the marker is revealed, or a list item reads `• - Buy milk`. The reveal plugin marks the construct (`revealedNode` on a `RevealSpan`) and `globals.css` does the rest — except for a node view, which ProseMirror hands its decorations rather than applying them to its DOM, and which does not re-render when they change. That is why `use-marker-revealed.ts` still exists for the task item's checkbox.
- `bulletList`/`orderedList`/`blockquote` still synthesize their marker at save time exactly as `prosemirror-markdown` always has, unlike every other construct's identity serialize, so `render-without-marker.ts` strips the real one from the content first. The wrapping node has to stay the sole writer: `renderList`'s `firstDelim` write and `wrapBlock`'s delimiter extension happen in a fixed order, and only a non-empty first write marks the output as no longer blank — an empty one leaves the next line's continuation indent applied to what should be the first line.
- List _kind_ is never re-derived from retyping a marker: the sync plugin corrects an absent or wrong-numbered one, but changing what an item structurally is would mean splitting it out of its list, out of scope.
- The image's source field is the only editable form an image has, so clearing it deletes the node. Text that is merely unparseable reverts — a half-typed `![alt](` is not a request to delete anything, and conflating the two makes an image impossible to remove by editing it.
- Not supported: merged cells (fall back to raw HTML), footnotes, underline/highlight/sub/sup — and inline nodes with content, which ProseMirror itself does not support. Typing inside one destroys its content and leaves it unreachable, reproduced with and without a node view; the image's `![alt](src)` is a separate controlled field for exactly this reason. The only route to real text there is plain text plus a widget decoration, which costs the `NodeSelection` the bubble menu, drag-and-drop and Tab-between-images all depend on.
...
- Adding a rule backed by a retext plugin touches four places past the id in `src/shared/messages.ts` — `RULES`, `RULE_PLUGINS`, `RULE_SOURCES`, and a test — all keyed by `TextToolRuleId` so a miss fails to compile, except `RULE_SOURCES` (reversed at runtime), where a miss silently reports nothing. `rule-kinds.ts` holds the two exclusion lists (`PluginRuleId`/`CustomPassRuleId`) both files derive from. A rule with no retext plugin at all (`polarity`, `dashOveruse`) skips both and builds `TextIssue`s directly in its own pass file.
- `retext-syntax-urls` is a parser extension, not a post-parse tree transform - it has to sit on the same processor that calls `.parse()` in `run-pipeline.ts`, not a later `.run()`.
- A technical-terms wordlist (`scripts/generate-technical-terms.ts`, from `@cspell/dict-software-terms`, a devDependency only) is spliced into every English Hunspell dictionary by `dictionaries/technical-terms.ts`.

### MCP server (`src/mcp/`)

`src/mcp/README.md` carries the why. The build is the only trap not in it: its own process, so unlike `agent-sdk-bundle.ts` it needs **no** `.cjs` loader shim — that shim exists because the CommonJS host has to reach an ES module, while this is a fresh `node` VSCode starts. Excluded from `tsconfig.host.json` and bundled separately (`vite.mcp.config.ts`), since it has to vendor its full dependency graph (the retext stack, hunspell dictionaries) rather than assume anything is installed.
...
- **Leave persistence to VS Code.** The webview only _syncs_ — a debounced `applyEdit` (`useDebounceValue`, immediate for the first edit since a clean state) that dirties the `TextDocument` and nothing more; VS Code decides when that reaches disk, via the keystroke, Save All, `files.autoSave`, or the close prompt. `onWillSaveTextDocument` (`save-participant.ts`) is what makes any of those exact rather than up to a debounce stale, by asking the active view for its current text before the save proceeds. Two things are forced by being a webview rather than a genuine editor extension, and nothing else should be: the sync itself, since the webview is the only holder of the edited document; and `retainContextWhenHidden` (`markdown-editor-provider.ts`), since VS Code would otherwise tear down a backgrounded tab's webview and take TipTap's undo history with it.
