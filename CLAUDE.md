...
- Every **block** construct's leading marker — a heading's `#`s, a blockquote's `> `, a list item's bullet/number/checkbox — goes through `block-marker/` instead: one `BlockMarkerSpec` per construct in `specs.ts`, and one shared implementation each of repair, reveal, caret boundary, Backspace-exits, toggle-off strip and paste. Adding a construct means adding a spec, not six files; a defect fixed in the shared pass is fixed for all of them, which is the whole reason the directory exists.
