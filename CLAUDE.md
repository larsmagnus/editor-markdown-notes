...
- `heading/` and `link/` extend the same "syntax becomes real, caret-revealed text" pattern as bold/strike/italic/code (see `formatting/`) to headings' `#`x`level` marker and links' `[text](href "title")` — read those directories' own doc comments for the pattern-specific gotchas.
- `list/`'s bullet/ordered/task marker is real leading text inside each item's own first paragraph, the same pattern - but `bulletList`/`orderedList` keep synthesizing that text at save time exactly like `prosemirror-markdown` always has, unlike every other construct's identity serialize: `state.renderList`'s `firstDelim` writes its marker string _before_ extending `this.delim` for continuation lines, and only a non-empty write there marks the output as "no longer blank" first - an empty `firstDelim` (real content instead) leaves nothing to do that, so the next line's indent gets wrongly applied to what should be the first line. `listItem`/`taskItem`'s own serialize (`strip-list-marker-for-serialize.ts`) just has to not render its own copy of what the wrapping list already writes. List _kind_ (bullet vs ordered vs task) is never re-derived from retyping the marker - `list-marker-sync-plugin.ts` corrects an absent or wrong-numbered marker, but changing what kind an item structurally is would mean splitting it out of its current list, out of scope here.
