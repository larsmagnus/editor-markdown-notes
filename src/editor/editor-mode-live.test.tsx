...
describe('Editor Mode Live', () => {
	...
	it('points images at a vscode-resource URI without rewriting the markdown', async () => {
		...
		render(<EditorModeLive content="![Architecture](./diagram.png)" />)
	...
	it('reports the parse error when a mermaid block is invalid', async () => {
		...
		render(
			<EditorModeLive content={['```mermaid', 'graph ??', '```'].join('\n')} />
		)
	...
	it('renders a link for a bare URL', async () => {
		render(<EditorModeLive content="Docs live at https://example.com today." />)
	...
	describe('syncing the content prop', () => {
		...
		it('keeps the document when the host echoes back equivalent markdown', async () => {
			...
			rerender(<EditorModeLive content={'# Roadmap\n\nShip it. Today.'} />)
		...
		it('survives switching to a note with fewer mermaid blocks', async () => {
			...
			const { rerender } = render(<EditorModeLive content={twoDiagrams} />)
			...
			rerender(
				<EditorModeLive content={`${MERMAID_NOTE}\n\nOne diagram left.`} />
			)
